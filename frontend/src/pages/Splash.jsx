    ...
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", background:"#fff", fontFamily:"'Nunito',sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ padding:"28px 22px 4px", position:"relative" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:9, marginBottom:6 }}>
          ...
          <h1 style={{ fontSize:23, fontWeight:900, color:C.text, letterSpacing:-0.5 }}>Sloty</h1>
        </div>
        <p style={{ fontSize:12, color:C.muted, fontWeight:700, textAlign:"center" }}>Skip the wait. Book your slot.</p>
      ...
      {/* Welcome heading + both cards + the legal links are grouped
          into one block and centered in whatever space is left
          between the (now more compact) header and the footer, rather
          than sitting pinned right under the header — "lower the cards
          toward center" only really works if this whole group moves as
          one unit, not just the cards on their own. */}
      ...
      {/* The middle block above already claims all the leftover space
          via flex:1 + justifyContent:center, so the footer just needs
          its own natural height here — it lands flush at the bottom
          with no gap on any screen size, without needing to stretch
          itself (which on a very tall phone made this band awkwardly
          oversized). */}
